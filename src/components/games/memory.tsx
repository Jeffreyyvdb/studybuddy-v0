...
export default function Memory() {
  ...
  const initializeGame = () => {
    ...
    const cardPairs = [...emojisToUse, ...emojisToUse].map((emoji, index) => ({
      id: index,
      emoji,
      isFlipped: false,
      isMatched: false,
  ...
  return (
    <div className="flex flex-col">
      {gameComplete ? (
        <div className="flex flex-col items-center justify-center p-8 bg-secondary rounded-lg shadow-md">
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-4">
          {cards.map((card) => (
            ...
            >
              {(card.isFlipped || card.isMatched) && (
                <span className="text-card-foreground">{card.emoji}</span>
              )}
            </Card>
          ))}
        </div>
